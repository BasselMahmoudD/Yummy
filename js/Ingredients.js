...
function displayData() {
  ...
  for(let i = 0; i < meals.length; i++) {
    meal += `<div class="col-md-3 my-2" >
        <div class="text-light text-center p-4 ms-5 img d-flex flex-column" role="button" onclick="getMealsByIngredient('${
...
async function getMealsByIngredient(Ingredient) {
  ...
  let finalRes = await response.json();
  meals = finalRes.meals;
  displayDataByIngredient();
}

function displayDataByIngredient() {
  let meal = ``;
