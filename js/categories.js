...
function displayData() {
  ...
  for (var i = 0; i < meals.length; i++) {
    meal += `<div class="col-md-3 my-2" >
      <div class="img" role="button" onclick="getMealsByCategories('${meals[i].strCategory}')">
          <img src="${meals[i].strCategoryThumb}" alt="meal" class="w-100">
          <div class="layer text-center meal"><h2>${meals[i].strCategory}</h2>
          <p class="text-dark">${meals[i].strCategoryDescription.substr(1,200)}</p></div>
      </div>
  </div>`;
  }
...
async function getMealsByCategories(meal) {
  let response = await fetch(
    `https://www.themealdb.com/api/json/v1/1/filter.php?c=${meal}`
  );
  let finalRes = await response.json();
  meals = finalRes.meals;
  displayDataByCategories();
}

function displayDataByCategories() {
  let meal = ``;
  for (var i = 0; i < meals.length; i++) {
    meal += `   <div class="col-md-3 my-2">
